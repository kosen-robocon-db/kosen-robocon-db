class AdvancementHistoriesController < ApplicationController
  ...
  def update
    if @robot.advancement_history.update_attributes(advancement_history_params) then
  ...
  def index
    advanced_histories = AdvancementHistory.all.order("robot_code ASC")
    respond_to do |format|
      format.csv { send_data advanced_histories.to_csv }
    end
  end
