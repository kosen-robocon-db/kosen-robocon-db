class GameDetail30th < GameDetail
  ...
  REX    = /-/
  REX_B  = /([0-9]|10|#{Constant::UNKNOWN_VALUE})/
  REX_BS = /#{REX_B}-#{REX_B}/
  ...
  def decompose_properties(robot:)
    ...
    self.my_robot_baloon, self.opponent_robot_baloon =
      h["robot-baloon"].to_s.split(REX) if h["robot-baloon"].present?
    self.my_base_baloon, self.opponent_base_baloon =
      h["base-baloon"].to_s.split(REX) if h["base-baloon"].present?
    ...
    roots = %w( robot_baloon base_baloon jury_votes )
