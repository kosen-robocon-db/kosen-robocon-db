class GameDetail1st < GameDetail
  ...
  REX_TIME  = /#{REX_MS}\.#{REX_MS}-#{REX_MS}\.#{REX_MS}/
  ...
  with_options if: :my_opponent_second? do
  ...
  def self.compose_properties(hash:)
    ...
    logger.debug(">>>> compose_properties:#{h.to_yaml}")
    return h
  end
    ...
    if h["time"].present? then
      self.my_time_minute, self.my_time_second,
        self.opponent_time_minute, self.opponent_time_second =
          h["time"].to_s.split(REX_TIME) # この時点ではREにマッチしたとしている
    end
